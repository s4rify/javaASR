...
public class UT_ASR_Proc_Utils {
	...
	public void testXcovComputation() throws IOException {
		// arrange
		double[][] filtpadded = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "filtpadded_small.csv");
	...
	public void testXcovComputationAfterIndexing() throws IOException {
		// arrange
		double[][] Xcov = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "Xcov1_small.csv");
		
		// act
		int stepsize = 32;
		int C = 4; // small:4, normal: 24
		double[][][] tmpXcov = Proc_Utils.extract_Xcov_at_stepsize(Xcov, stepsize, C);
		
		// assert
		double[][] actualXcov = Proc_Utils.computeCurrentXcov(C, tmpXcov, 0);
		double[][] expectedXcov = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "Xcov2_small" + 1 + ".csv");
		TestUtils.assertDeepEquals(expectedXcov, actualXcov, 0.1); 
	...
	public void eigenDecompTest_eigenvectorsV() throws IOException {
		int C = 24;
		// matlab only writes some files currently, hence the low loop couter
		for (int i = 1; i < 2; i++) {
	 ...
	 */
	@Test
	public void provide_M_andComputeTheRest() throws IOException{
		double[][] M = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "M_small.csv");
		provide_M_or_T_andComputeTheRest(M, true);
	}

	@Test
	public void provide_T_andComputeTheRest() throws IOException{
		double[][] T = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "T_small.csv");
		provide_M_or_T_andComputeTheRest(T, false);
	}

	private void provide_M_or_T_andComputeTheRest(double[][] precompMatrix, boolean calculateT) throws IOException{
		// arrange
		double[][] processData = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "process_data_small.csv");
		double[][] calibData = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "calib_data_small.csv");
		
		// act
		int sampleRate = 250;
		ASR_Calibration calibState = new ASR_Calibration(calibData, precompMatrix, calculateT);
		
		ASR_Process proc = new ASR_Process(calibState, sampleRate);
		double[][] actualCleanData = proc.asr_process(processData);
		
		// assert
		double[][] expectedCleanData = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "cleanData_small.csv");
	...
	public void smokeTestComplete() throws IOException{
		...
		CsvWriter.writeMatrixToPath(actualCleanData, exportPath, "actualCleanData_small");
	...
	public void testDataReconstruction() throws IOException {
		...
		double[][] M = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "M_small.csv");
		double[][] onlypadded = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "onlypadded_small.csv");
		...
		for (int u = 0; u < stepsize; u++) {
			double[][] curV = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "V_all_sorted_small" + (u+1) + ".csv");
			double[] keep = CSVReader.readDoublesFromFile(FIRST_TEST_DATA_FOLDER + "KEEP_small" + (u+1) + ".csv");
        ...
        double[][] expectedOutData = CSVReader.readDoublesFromFile2d(FIRST_TEST_DATA_FOLDER + "cleanData_small.csv");
