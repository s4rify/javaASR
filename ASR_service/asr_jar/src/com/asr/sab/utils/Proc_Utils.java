...
public class Proc_Utils {
	...
	private static final IFilter filter = FilterFactory.getFilter();
	...
	public static double[][][] extract_Xcov_at_stepsize(double[][] Xcov, int stepsize, int C) {
        int len1 = Xcov.length;
        //min(stepsize:stepsize:(size(Xcov,2)+stepsize-1),size(Xcov,2))
        int len2 = (Xcov[0].length / stepsize)+1;
        
        if (len1 < 1) len1 = 1;
        if (len2 < 1) len2 = 1;
		double[][] indexedXcov = new double[len1][len2];
		int s = 0;
	...
	public static double[] compute_threshold_operator(int C, double[][] T, double maxdims, double[][] V, double[] D) {
		...
		for (int j = 0; j < TV[0].length; j++) {
			if(Math.abs(D[j]) < sumsqTV[j] || channelRange[j] < dim){  
				keep[j] = IS_CLEAN;
			} else {
	 ...
	 */
	public static double[][] reconstruct_data(int C, double[][] data,  double[][] R, double[][] last_R, int[] subrange) {
		 ...
		 * we take blend directly from Matlab, since it does not depend on the data, btut only on stepsize, which will be the same for
		 * our use cases for now.
		 */
		double [] blend = {0.00240763666390154,0.00960735979838479,0.0215298321338956,0.0380602337443566,0.0590393678258225,0.0842651938487274,0.113494773318632,0.146446609406726,0.182803357918177,
				0.222214883490199,0.264301631587001,0.308658283817455,0.354857661372769,0.402454838991936,0.450991429835220,0.500000000000000,0.549008570164780,0.597545161008064,
				0.645142338627231,0.691341716182545,0.735698368412999,0.777785116509801,0.817196642081823,0.853553390593274,0.886505226681368,0.915734806151273,0.940960632174178,
				0.961939766255643,0.978470167866104,0.990392640201615,0.997592363336098,1};
		// note to self: this does not apply for 100 sr data!!
	 ...
	 */
	public static double[][] filter_data(double[][] data, double sRate) {
		return filter.filter_data(data, sRate);
	}
