...
import com.asr.sab.filter.FilterOldImpl;
...
//    		assert y.length == channelPairCount : "Wrong dimension: y should have an entry for each pair of channels.";
    ...
    public static double[][] calculate_RMS (double[][] X, double window_overlap, int N) {
    	...
    	double[] interim = new double[win[0].length];
    	int in = 0;
    	...
    	for (int c = 0; c < C; c++) {
    		for (int i = 0; i < win.length; i++) {
    			...
    			for (int i1 = 0; i1 < win[0].length; i1++) {
    				// divide by N
    				interim[i1] = interim[i1]/N;
    				// add up the columns in interim
    				tmp += interim[i1];
    			}
    			rms[c][i] = Math.sqrt(tmp);
    			in = 0;
    			tmp = 0;
    		}
    		in = 0;
